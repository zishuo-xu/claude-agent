...
- 结合 `ConversationFocus` 让短 follow-up 优先参考当前会话焦点
- 结合 `WorkingState` 判断当前 intent，支持澄清问题和继续追加场景下的短期任务延续
...
会话焦点边界，对应 Claude-style agent 中“完整对话历史参与下一步判断”的轻量实现：

- 只在当前 runtime 内记录一个焦点，不跨会话持久化
- 当前焦点可为 project、content、coding、general
- 短 follow-up 先参考焦点，再落到关键词规则
- 内容生成焦点下，“输出为文档 / 整理成文档”等会使用对话已有内容，不读取项目文件

它不是长期记忆，也不是 LLM intent classifier；它只负责减少 `classify_intent()` 的硬语义判断。

### `mini_agent.working_state`

短期工作状态边界，对应 Claude-style agent loop 中“当前任务状态参与下一轮决策”的简化形态：

- 只记录一个 pending task
- 只在当前 runtime 内有效，不持久化
- 当 coding task 的最终回复明显是在等待用户补充时进入 waiting 状态
- 下一轮如果用户输入像补充参数，则继承上一轮 intent 和工具可见性
- 只读工具不会被当成任务完成；写入类工具会标记为 mutating，避免无边界继承
- 用户取消或切换到明确新任务时，会清空 pending 状态

当前不做长期记忆、session resume、多任务队列或复杂 planner。
