...
- Context：tool result budget、context preflight、micro-compact、full compact、超预算阻断、summary 注入。
...
- `0.27.0` 已引入 ConversationFocus，`0.27.1` 已修复保存到路径和项目焦点下整理成文档的问题，`0.27.2` 已修复保存一下和取消后继续的问题，`0.27.3` 已修复切换项目问答后明确回到文件续写的问题，`0.27.4` 已补强严格步骤遵循提示，`0.27.5` 已补强最终验证后停止提示，`0.27.6` 已修复 CLI 粘贴多行输入被拆成多轮的问题，`0.27.7` 已修复混合项目问答和文件动作的工具门控，`0.27.8` 已修复模型流式错误导致 CLI traceback 退出的问题，`0.27.9` 已阻止系统级 Python 包安装命令，`0.27.10` 已强制工作区优先使用 `.venv/bin/python -m ...`，`0.27.11` 已修复项目问答验收场景误走 coding task，`0.27.12` 已清理文件写入中的 thinking 标记，`0.27.13` 已阻止 shell here-doc 写文件正文，`0.27.14` 已收敛 shell 大输出展示。
