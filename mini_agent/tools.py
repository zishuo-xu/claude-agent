...
def default_tools(root: Path, task_state: TaskState | None = None) -> dict[str, Tool]:
    ...
    def edit_file(args: dict[str, Any]) -> str:
        ...
        updated, occurrences = replace_text(content, old, new, args.get("replace_all", False))
    ...
    def preview_edit(args: dict[str, Any]) -> str:
        path = workspace.resolve(args["path"])
        old = args["old"]
        new = args["new"]
        content = path.read_text(encoding="utf-8")
    ...
    def apply_edit(args: dict[str, Any]) -> str:
        path = workspace.resolve(args["path"])
        old = args["old"]
        new = args["new"]
        content = path.read_text(encoding="utf-8")
    ...
    def set_tasks(args: dict[str, Any]) -> str:
        return tasks.set_tasks(args["tasks"])

    def update_task(args: dict[str, Any]) -> str:
        return tasks.update_task(args["id"], args["status"], args.get("note", ""))

    def list_tasks(_args: dict[str, Any]) -> str:
        return tasks.render()
